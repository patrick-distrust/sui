...
Commits after `fe3abe3a8c (VM Limiter Bypass)` introduce a timestamp-aware “V2” bridge flow that temporarily bypasses the 24 h limiter for newly approved transfers (≤48 h old). The rest of this primer documents the baseline implementation; refer to this section whenever you need the delta.

### Move-Side Additions

- `TokenDepositedEventV2` and `send_token_v2` mirror the original deposit paths but append `timestamp_ms`, allowing downstream components to prove freshness:
...
- `TokenTransferPayloadV2` and `BridgeMessage::create_token_bridge_message_v2` carry the timestamp over the wire; `claim_token_internal` now checks that value and bypasses `limiter.check_and_record_sending_transfer` if the claim happens within 48 hours of emission:
...
- `SuiBridgeV2.sol` duplicates the deposit/claim entry points with timestamp-aware variants. Deposits emit `TokensDepositedV2` (including `block.timestamp`), while withdrawals call `_transferTokensFromVault` guarded by `limitNotExceededV2`, which accepts the timestamp from the signed payload:

```16:66:bridge/evm/contracts/SuiBridgeV2.sol
function transferBridgedTokensWithSignaturesV2(/* ... */) external nonReentrant {
...
- `bridgeERC20V2`/`bridgeETHV2` emit `TokensDepositedV2` with the wall-clock time so the Move side can bypass its limiter symmetrically.
